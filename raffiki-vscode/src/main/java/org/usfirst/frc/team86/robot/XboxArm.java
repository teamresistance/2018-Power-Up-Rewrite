...
public class XboxArm implements IUpdate{
	...
	public XboxArm(TalonSRX armMotor, VictorSP leftIntake, VictorSP rightIntake, ISolenoid flippers, ISolenoid claw, ISolenoid armBrake){
...
//		SmartDashboard.putNumber("Arm Deadband", 0);
	...
	public void update() {
		...
		if (rollersEnabled) {
			int POVval = JoystickIO.xBox.getPOV();
			switch(POVval) {
			case 0:
				leftIntake.set(-1);
				rightIntake.set(1);
...
//		}
		...
		case 0:
			...
			if (JoystickIO.xBox.getRawAxis(2)>.5) {
				flippers.set(true);
				rollersEnabled = true;
			}
			// close flippers on release
			if (JoystickIO.xBox.getRawAxis(2)<.5 || IO.cubePartiallyIn.get()) {
				flippers.set(false);
				state = 1;
			}
			break;
		...
		case 5:
			...
			if (Time.getTime() - initialTime >= unGrabCubeDelay) {
				if (JoystickIO.xBox.getPOV() == 0) {
					leftIntake.set(-1);
					rightIntake.set(1);
				} else if (JoystickIO.xBox.getPOV() == 180) {
					leftIntake.set(1);
					rightIntake.set(-1);
				} else {
					leftIntake.set(0);
